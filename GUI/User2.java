...
public class User2 extends JFrame implements ActionListener {
    ...
    public void UserMessageBox(MessageBox box) {
        // สร้าง panel เพื่อวางกล่องข้อความ โดยใช้ BorderLayout ภายใน
        JPanel msgPanel = new JPanel(new BorderLayout());
        msgPanel.add(new JScrollPane(box.getTextArea()), BorderLayout.CENTER);
        msgPanel.setBorder(BorderFactory.createTitledBorder("กล่องข้อความของ User"));
        
        // กำหนดตำแหน่งมุมขวาบน (ปรับขนาด/ตำแหน่งตามต้องการ)
        // ค่าเหล่านี้ใช้กับหน้าต่างขนาด 400x300 ตามที่ตั้งไว้ใน Finally()
        msgPanel.setBounds(230, 10, 160, 150);
        
        // เพิ่มลงใน content pane ที่ใช้ null layout
        cp.add(msgPanel);
        cp.revalidate();
        cp.repaint();
    }
